...
import 'src/airoports.dart' as airoPlaces;
import 'dart:math' as math;

void main() {
  runApp(const AiroLocations());
}

class AiroLocations extends StatefulWidget {

  const AiroLocations({Key? key}) : super(key: key);
...
class _MyAppState extends State<AiroLocations> {
  double range=3000;
  ...
  Future<void> _onMapCreated(GoogleMapController controller) async {
    ...
    final places = await airoPlaces.getPlaces();
    setState(() {
      _markers.clear();
      for (final place in places.airoports) {
        print("Airos" + place.code + " " + place.name);
        final marker = Marker(
          markerId: MarkerId(place.code),
          position: LatLng(num.parse(place.lat).toDouble(), num.parse(place.lon).toDouble()),
          infoWindow: InfoWindow(
              title: place.name,
              snippet: place.country
          ),
        );
        _markers[place.code] = marker;
      }
    });
  }
